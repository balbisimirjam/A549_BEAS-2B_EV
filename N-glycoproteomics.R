...
source(file="functions/glycan_monomer_extractor.R")

# read in GlycReSoft results files ####
setwd("D:/EV_GP_241213/250130_glycresoft/glycresoft_output")
...
setwd("D:/EV_GP_241213/250130_glycresoft/results")
# write.csv(full_matrix, file = "imputed_full_matrix_315GP.csv", row.names = T)
...
# write.csv(statistic_results, file = "statistic_results_315GP.csv")
...
# calculate glycan type based on monomers
glycan_composition$glycan_type <- "Misc"
glycan_composition$glycan_type[glycan_composition$HexNAc > 3 & glycan_composition$Hex > 2] <- "Complex"
glycan_composition$glycan_type[glycan_composition$HexNAc == 3 & glycan_composition$Hex > 3] <- "Hybrid"
glycan_composition$glycan_type[glycan_composition$HexNAc == 2 & glycan_composition$Hex > 3] <- "Oligomannose"
...
glycan_composition[, 7:18] <- lapply(glycan_composition[, 7:18], function(x) as.numeric(as.character(x)))

# Multiply glycopeptide intensity by Hex, Neu5Ac, Fuc, and HexNAc numbers for each sample
weighted_neu5ac <- colSums(glycan_composition[, colnames(sample_data)] * (glycan_composition$Neu5Ac), na.rm = TRUE) / 
  colSums(glycan_composition[, colnames(sample_data)] * (glycan_composition$HexNAc - 2 - (1 - (glycan_composition$Hex - glycan_composition$HexNAc))), na.rm = TRUE)
weighted_neu5ac_per_glycan <- colSums(glycan_composition[, colnames(sample_data)] * (glycan_composition$Neu5Ac), na.rm = TRUE) / 
...
weighted_fuc <- colSums(glycan_composition[, colnames(sample_data)] * (glycan_composition$Fuc), na.rm=TRUE) / 
  colSums(glycan_composition[, colnames(sample_data)], na.rm = TRUE)
weighted_gal <- colSums(glycan_composition[, colnames(sample_data)] * (glycan_composition$Hex - 3), na.rm=TRUE) / 
  colSums(glycan_composition[, colnames(sample_data)] * (glycan_composition$HexNAc - 2), na.rm = TRUE)
weighted_gal <- as.matrix(weighted_gal)
weighted_gal[is.infinite(weighted_gal)] <- 0

complex_ratio <- colSums(glycan_composition[glycan_composition$glycan_type == "Complex", colnames(sample_data)], na.rm=TRUE) / 
  colSums(glycan_composition[, colnames(sample_data)], na.rm = TRUE)
oligomannose_ratio <- colSums(glycan_composition[glycan_composition$glycan_type == "Oligomannose", colnames(sample_data)], na.rm=TRUE) / 
  colSums(glycan_composition[, colnames(sample_data)], na.rm = TRUE)
hybrid_ratio <- colSums(glycan_composition[glycan_composition$glycan_type == "Hybrid", colnames(sample_data)], na.rm=TRUE) / 
  colSums(glycan_composition[, colnames(sample_data)], na.rm = TRUE)
misc_ratio <- colSums(glycan_composition[glycan_composition$glycan_type == "Misc", colnames(sample_data)], na.rm=TRUE) / 
  colSums(glycan_composition[, colnames(sample_data)], na.rm = TRUE)

average_glycan_df <- data.frame(
  weighted_gal,
  weighted_neu5ac,
  # weighted_neu5ac_per_glycan,
  weighted_fuc,
  complex_ratio,
  oligomannose_ratio,
  hybrid_ratio,
...
ggplot(average_glycan_df, aes(x = group, y = complex_ratio*100, fill = group)) +
  geom_boxplot() +
...
#heatmap for glycans
glycan_composition_summarized <- glycan_composition %>%
  group_by(Hex, HexNAc, Neu5Ac, Fuc) %>%
  summarize(
    # Sum intensities for all samples (A1, A2, ..., B6, etc.)
    across(starts_with("A"), sum, na.rm = TRUE), 
    across(starts_with("B"), sum, na.rm = TRUE), 
    glycan_type = first(glycan_type), 
    .groups = "drop" 
  )

glycan_composition_summarized <- glycan_composition_summarized %>%
  mutate(glycan_name = paste0("H", Hex, "N", HexNAc, "S", Neu5Ac, "F", Fuc))

# Prepare the data for heatmap
# Select the glycan intensities (columns starting with 'A' and 'B') and the new glycan name
heatmap_data <- glycan_composition_summarized %>%
  select(glycan_name, starts_with("A"), starts_with("B")) %>%
  column_to_rownames("glycan_name")

# Scale the data (rows) for better visualization
heatmap_data_scaled <- t(scale(t(heatmap_data), center = TRUE, scale = TRUE))
...
# Set color breaks for better control over the color scale
breaks <- quantile(as.matrix(heatmap_data_scaled), probs = seq(0, 1, 0.025), na.rm = TRUE)
ramp <- colorpanel(low = left.color, high = right.color, n = 40)
...
heatmap.2(
  ...
  key.title = "", 
  key.ylab = "",
  hclustfun = hm_function, 
  ColSideColors = as.character(group_colors),
  labCol = paste(full_matrix$group),  # Assuming group information is in the full_matrix
  margins = c(8, 1),
...
# clustering
hc_rows <- hclust(dist(heatmap_data_scaled), method = "ward.D2")
row_clusters <- cutree(hc_rows, k = 2)
glycan_cluster_df <- data.frame(glycan_name = rownames(heatmap_data_scaled), cluster = row_clusters)

# write.csv(average_glycan_df,"average_glycan_df.csv")
write.csv(glycan_composition_summarized,"glycan_composition_summarized.csv")
